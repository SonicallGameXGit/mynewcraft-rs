...
impl Shader {
    ...
    pub fn set_vec2(&self, name: &str, value: (f32, f32)) {
        unsafe { gl::Uniform2f(self.get_uniform_location(name), value.0, value.1); }
    }
    pub fn set_ivec2(&self, name: &str, value: (i32, i32)) {
        unsafe { gl::Uniform2i(self.get_uniform_location(name), value.0, value.1); }
    }
    pub fn set_vec3(&self, name: &str, value: (f32, f32, f32)) {
        unsafe { gl::Uniform3f(self.get_uniform_location(name), value.0, value.1, value.2); }
    }
    pub fn set_mat4(&self, name: &str, value: &cgmath::Matrix4<f32>) {
